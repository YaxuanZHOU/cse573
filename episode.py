...
class Episode:
    ...
    def state_for_agent(self):
        ...
        memory = list(np.array(self.envSuccess).astype(float))
    ...
    def judge(self, action):
        ...
        if action['action'] == 'FoundTomato':
            objects = self._env.last_event.metadata['objects']
            visible_objects = {o['objectType']: o['objectId'] for o in objects if o['visible']}
            if 'Tomato' in list(visible_objects.keys()) and self.envSuccess[0] == False:
                self.tomatoID = visible_objects['Tomato']
                tomatoSuccess = self.environment.tomatoEnvSuccess(self.tomatoID)
                if tomatoSuccess:
                    self.envSuccess[0] == True
                    reward += GOAL2_SUCCESS_REWARD

        if action['action'] == 'FoundMicrowave':
            objects = self._env.last_event.metadata['objects']
            visible_objects = {o['objectType']: o['objectId'] for o in objects if o['visible']}
            if 'Microwave' in list(visible_objects.keys()) and self.envSuccess[1] == False:
                microwaveSuccess = self.environment.microwaveEnvSuccess(visible_objects['Microwave'], self.tomatoID)
                if microwaveSuccess:
                    self.envSuccess[1] = True
                    reward += GOAL_SUCCESS_REWARD

        if action['action'] == 'Done':
            done = True
            self.success = self.envSuccess[0] and self.envSuccess[0]
            if self.success:
                reward += GOAL_SUCCESS_REWARD
